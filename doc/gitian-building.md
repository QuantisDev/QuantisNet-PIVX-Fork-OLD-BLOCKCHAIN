...
*Setup instructions for a gitian build of QuantisNET using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the QuantisNET
...
- [Building QuantisNET](#building-quantisnet)
...
Clone the git repositories for quantisnet and gitian and then checkout the quantisnet version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the quantisnet repository
...
To build QuantisNET (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the quantisnet repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/quantisnet/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/eastcoastcrypto/QuantisNET
...
For example:
```bash
URL=https://github.com/eastcoastcrypto/QuantisNET.git
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/quantisnet-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/quantisnet-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/quantisnet-build.assert
```
