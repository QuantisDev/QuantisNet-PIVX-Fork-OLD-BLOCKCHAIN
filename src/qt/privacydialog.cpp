...
#include "zquantisnetcontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zQUAN ought to be enough for anybody." - Bill Gates, 2017
    ui->zQUANpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzQUANSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzQUAN->setEnabled(false);
        ui->pushButtonMintzQUAN->setToolTip(tr("zQUAN is currently disabled due to maintenance."));

        ui->pushButtonSpendzQUAN->setEnabled(false);
        ui->pushButtonSpendzQUAN->setToolTip(tr("zQUAN is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zQUANpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzQUAN_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zQUAN is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zQUAN...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zQUAN in ") +
...
void PrivacyDialog::on_pushButtonSpendzQUAN_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zQUAN is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zQUAN
        sendzQUAN();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zQUAN
    sendzQUAN();
}

void PrivacyDialog::on_pushButtonZQuantisNETControl_clicked()
{
    ZQuantisNETControlDialog* zQuantisNETControl = new ZQuantisNETControlDialog(this);
    zQuantisNETControl->setModel(walletModel);
    zQuantisNETControl->exec();
}

void PrivacyDialog::setZQuantisNETControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzQuantisNETSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzQUAN()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid QuantisNET Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zQUANpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zQUAN is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " QUAN </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zQUAN</b>";
    ...
    // use mints from zQuantisNET selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZQuantisNETControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZQuantisNETControlDialog::GetSelectedMints();
    }
    ...
    // Clear zquantisnet selector in case it was used
    ZQuantisNETControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zQuantisNET Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " QuantisNET, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zQuantisNET Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zQUAN </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zQUAN "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zQUAN "));
    ui->labelzQUANAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzQUANSyncStatus->setVisible(fShow);
}
