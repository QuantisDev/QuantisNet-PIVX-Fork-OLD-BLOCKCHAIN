...
#include "ui_zquantisnetcontroldialog.h"
...
std::list<std::string> ZQuantisNETControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZQuantisNETControlDialog::listMints;

ZQuantisNETControlDialog::ZQuantisNETControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZQuantisNETControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZQuantisNETControlDialog::~ZQuantisNETControlDialog()
{
    delete ui;
}

void ZQuantisNETControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZQuantisNETControlDialog::updateList()
{
...
void ZQuantisNETControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZQuantisNETControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZQuantisNET_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZQuantisNETControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZQuantisNETControlDialog::GetSelectedMints()
{
...
void ZQuantisNETControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
