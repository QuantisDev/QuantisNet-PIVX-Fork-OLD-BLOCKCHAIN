...
#if defined(NDEBUG)
#error "QuantisNET cannot be compiled without assertions."
#endif
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zQuantisNET spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
                    return state.Invalid(error("%s : zQuantisNET spend with serial %s is already in block %d\n",
                ...
                if (!spend.HasValidSerial(Params().Zerocoin_Params()))
                    return state.Invalid(error("%s : zQuantisNET spend with serial %s from tx %s is not in valid range\n",
...
int64_t GetBlockValue(int nHeight)
{
    int64_t nSubsidy = 0;
      ...
      if (nHeight == 0) {
        nSubsidy = 4700000 * COIN;
      } else if (nHeight <= 2500 && nHeight > 0) {
        nSubsidy = 0.001 * COIN;
      } else if (nHeight <= 12501 && nHeight > 2500) {
          nSubsidy = 30 * COIN;
      } else if (nHeight <= 512502 && nHeight > 12501) {
          nSubsidy = 5 * COIN;
      } else if (nHeight <= 1012503 && nHeight > 512502) {
          nSubsidy = 6 * COIN;
      } else if (nHeight <= 1512504 && nHeight > 1012503) {
          nSubsidy = 0.03 * COIN;
      } else if (nHeight <= 1762505 && nHeight > 1512504) {
          nSubsidy = 0.015 * COIN;
      } else if (nHeight <= 2012505 && nHeight > 1762505) {
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
	...
	if (nHeight <= 300) {
        ...
        } else if (nHeight <= 2500 &&  nHeight > 300) {
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from QuantisNET
...
{
    RenameThread("quantisnet-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZQUANMinted()
{
...
void RecalculateZQUANSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zQUAN supply
...
bool RecalculateQUANSupply(int nHeightStart)
{
                ...
                if (zerocoinDB->ReadCoinSpend(spend.getCoinSerialNumber(), hashTxFromDB)) {
                    if(IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
                        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
                            return state.DoS(100, error("%s : zQuantisNET with serial %s is already in the block %d\n",
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zQuantisNETSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert QUAN to zQUAN
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zQuantisNET spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zQuantisNET serial %s in block\n Block: %s",
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zQuantisNET mints and %d zQuantisNET spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // QuantisNET: We use certain sporks during IBD, so check to see if they are
